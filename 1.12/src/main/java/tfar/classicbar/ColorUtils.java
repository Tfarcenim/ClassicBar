...
public class ColorUtils {
    ...
    public static Color calculatePoisonedScaledColor(double d1, double d2) {
        double d3 = (d1 / d2);
        if (colors.advancedColors.poisonedFractions.length != colors.advancedColors.poisonedColors.length) return Color.BLACK;
        int i1 = colors.advancedColors.poisonedFractions.length - 1;
        int i3 = 0;
        for (int i2 = 0; i2 < i1; i2++) {
            if (d3 < colors.advancedColors.poisonedFractions[i2]) break;
            i3++;
        }
        ...
        //return last color in the list if health is too high
        if (d3 >= colors.advancedColors.poisonedFractions[colors.advancedColors.poisonedFractions.length - 1])
            return hex2Color(colors.advancedColors.poisonedColors[colors.advancedColors.poisonedFractions.length - 1]);
    ...
    public static Color calculateWitheredScaledColor(double d1, double d2) {
        double d3 = (d1 / d2);
        if (colors.advancedColors.witheredFractions.length != colors.advancedColors.witheredColors.length) return Color.BLACK;
        int i1 = colors.advancedColors.witheredFractions.length - 1;
        int i3 = 0;
        for (int i2 = 0; i2 < i1; i2++) {
            if (d3 < colors.advancedColors.witheredFractions[i2]) break;
            i3++;
        }
        ...
        //return last color in the list if health is too high
        if (d3 >= colors.advancedColors.witheredFractions[colors.advancedColors.witheredFractions.length - 1])
            return hex2Color(colors.advancedColors.witheredColors[colors.advancedColors.witheredFractions.length - 1]);
